...
if(media.length === 0){
    viewer.innerHTML = "<h1 style='color:white;text-align:center;margin-top:20%'>NO MEDIA</h1>";
    return;
}
    let item = media[i];
...
// ================= VIDEO =================
    ...
    // wajib muted dulu agar autoplay stabil
    el.muted = true;

    // play saat video siap
    el.addEventListener("canplay", () => {

        el.play().then(() => {

            // jika audio sudah diunlock
            if(audioUnlocked){
                el.muted = false;
    ...
    // lanjut media berikut setelah selesai
    el.onended = function(){
...
function reloadMedia(){
    ...
    fetch('load_media.php?_=' + new Date().getTime())
        ...
        .then(data => {
            ...
            // update lastId global
            lastId = media.length > 0
